...
terraform {
  required_providers {
    kkp = {
      ...
      version = "~> 0.1.12" # pin to a minor series
    }
  }
}
...
There is currently no `make testacc` target in this repository. The CI-aligned checks are:
...
2. Create the release tag:
   ...
   ```
3. Push the tag (this runs `docs-verify` first):
   ```bash
   make tag-push VERSION=vX.Y.Z
   ```
4. A GitHub Release will be created with binaries, checksums, and autogenerated release notes.
...
| `fmt` | Format Go code |
| `fmt-check` | Check Go/Terraform formatting |
| `tidy` | Tidy up go.mod |
...
| `docs` | Generate docs/snippets and verify they are in sync |
| `docs-verify` | Verify docs/snippets/examples match `VERSION` |
| `examples-validate` | Validate `examples/` with local provider binary |
| `tag` | Create annotated tag `VERSION=vX.Y.Z` |
| `tag-push` | Run docs verify and push tag |
| `tag-repush` | Recreate and push an existing tag |
